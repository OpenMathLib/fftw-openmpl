...
void CONSTRUCTOR openvml_memory_init()
{
    if (!is_thread_init())
        openvml_thread_init();
    // get the num of core
    int num_core = get_cpu_core();
    // map addr for buffer
    for (int i = 0; i < num_core; i++) {
        ...
        buffer[i].core = i;
        buffer[i].node =get_node_from_cpu(i);
        buffer[i].is_used = 0;
        buffer[i].offset = 0;
...
{
    if (buffer[core].offset + size - 1 > BUFFER_SIZE)
        return NULL; // the buffer size is not enough
...
void* openvml_get_free_buffer_addr(int core)
{
    if (buffer[core].offset > BUFFER_SIZE)
        return NULL; // the capacity of buffer is not enough
    void* buf_addr;
...
void openvml_memory_copy(void* ori_addr, void* dst_addr, size_t size)
{
    if (ori_addr == NULL)
        return;
    if (size == 0)
